...
function createTestHandler() {
  ...
  handler.detect.and.callFake((decorators: Decorator[]) => decorators.find(d => d.name === 'Component'));
  // The "test" analysis is just the name of the decorator being analyzed
  handler.analyze.and.callFake(((decl: ts.Declaration, dec: Decorator) => ({ analysis: dec.name, diagnostics: null })));
...
function createParsedFile(program: ts.Program) {
  ...
  file.decoratedClasses.push(new ParsedClass(
    'MyComponent',
    {} as any,
    [{ name: 'Component', import: { from: '@angular/core', name: 'Component' }, node: null as any, args: null}]
  ));
  ...
  file.decoratedClasses.push(new ParsedClass(
    'MyService',
    {} as any,
    [{ name: 'Injectable', import: { from: '@angular/core', name: 'Injectable' }, node: null as any, args: null}]
  ));
...
describe('Analyzer', () => {
  describe('analyzeFile()', () => {
    ...
    beforeEach(() => {
      ...
      const analyzer = new Analyzer(program.getTypeChecker(), new Esm2015ReflectionHost(program.getTypeChecker()));
    ...
    it('should call detect on the decorator handlers with each class from the parsed file', () => {
      ...
      expect(testHandler.detect.calls.allArgs()[0][0]).toEqual([jasmine.objectContaining({ name: 'Component' })]);
      expect(testHandler.detect.calls.allArgs()[1][0]).toEqual([jasmine.objectContaining({ name: 'Injectable' })]);
    });
