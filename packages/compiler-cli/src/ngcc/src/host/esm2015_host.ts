...
import { reflectObjectLiteral, TypeScriptReflectionHost } from '../../../ngtsc/metadata/src/reflector';
...
export class Esm2015ReflectionHost extends TypeScriptReflectionHost implements NgccReflectionHost {
  ...
  getConstructorParameters(clazz: ts.Declaration): Parameter[]|null {
    ...
    if (parameterNodes) {
      ...
      parameterNodes.forEach((node, index) => {
        ...
        const decorators = info && info.has('decorators') && this.reflectDecorators(info.get('decorators')!) || null;
    ...
    if (classSymbol.exports && classSymbol.exports.has(PROP_DECORATORS)) {
      ...
      if (propDecoratorsMap && ts.isObjectLiteralExpression(propDecoratorsMap)) {
        ...
        propertiesMap.forEach((value, name) => {
          memberDecorators.set(name, this.reflectDecorators(value));
        });
      }
    }
  ...
  protected reflectMember(symbol: ts.Symbol, decorators?: Decorator[], isStatic?: boolean): ClassMember|null {
    ...
    const node = symbol.valueDeclaration || symbol.declarations && symbol.declarations[0] as ts.ClassElement;
    if (!node) {
      return null;
    }
    ...
    if (isStatic) {
      ...
      value = symbol.flags & ts.SymbolFlags.Property ? (node.parent as ts.BinaryExpression).right : null;
